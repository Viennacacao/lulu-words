...
字里行间是一款为 macOS 设计的本地英语学习工具。它将单词学习藏进一张只读办公文档页面，帮助你在工作间隙中低调、连续地学习——词藏文档，学在字里行间。
...
文档第一页中部是学习区，默认六行；可在"我的"中切换为八行（八行模式下助记与短语各占两行，可显示更多内容）：
...
应用使用 FSRS 间隔重复算法，根据每个单词的难度、记忆稳定度、上次复习时间和用户评分计算下次复习时间。

每天的学习顺序为：

1. 收集所有已学词中今天到期或已经逾期的单词，不受当前词书限制。
2. 按预测遗忘风险排序，越容易忘记的词越先出现。
3. 安排完到期复习后，使用当前词书的新词补足“每日学习目标”。

例如每日目标为 30，今天有 18 个到期词，计划会加入 12 个新词。如果今天已有 36 个到期词，应用会优先完成这些复习，不再自动增加新词。

三个评分的含义：

- **忘记**：没有回忆出来。该词会在若干个其他单词后再次出现。
- **模糊**：已经回忆出来，但很犹豫或比较费力。
- **认识**：正常、明确地回忆出来。

显示答案后，评分按钮会预览不同选择对应的下次复习间隔。当天队列、完成位置和忘记后等待重学的单词会自动保存，退出后重新打开可继续原计划。切换词书会使用新词书重新生成当天队列，已经完成的复习记录不会重复安排。

### 背景文档

进入"文本"，点击"导入背景文档"，可选择 TXT 或 DOCX。应用会提取文字并将其排入只读页面，中间自动为学习区留出位置。

导入的背景文档会自动保存到本地（"文本"页的"已保存背景文档"列表，最多 50 篇），下次启动可直接打开，无需重新导入；删除只移除应用保存的副本，不影响你的原文件。
...
今日记忆计划由系统按遗忘风险排序，计划中不使用左右方向键跳过未评分单词。
...
间隔重复调度使用开源 [`ts-fsrs`](https://github.com/open-spaced-repetition/ts-fsrs)，遵循 MIT License。
